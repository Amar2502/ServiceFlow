...
export function NotificationBell() {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-80 sm:w-96 bg-popover border-border shadow-lg p-0">
        <div className="flex items-center justify-between p-3 border-b border-border bg-muted/60">
          <div className="flex items-center gap-2">
            <Bell className="h-4 w-4 text-foreground" />
            <h3 className="font-bold text-xs text-foreground">Assigned Ticket Notifications</h3>
            {unreadCount > 0 && (
          ...
          <div className="flex items-center gap-1">
            {unreadCount > 0 && (
              <Button
                variant="ghost"
                size="sm"
                className="h-7 text-[10px] text-foreground hover:bg-muted px-1.5"
            ...
            {notifications.length > 0 && (
              <Button
                variant="ghost"
                size="sm"
                className="h-7 text-[10px] text-red-700 hover:bg-red-100 dark:hover:bg-red-950/40 px-1.5"
                onClick={() => clearAll()}
            ...
            notifications.map((n) => (
              <DropdownMenuItem
                ...
                className={`p-3 cursor-pointer flex flex-col items-start gap-1 transition-colors ${
                  n.read ? "bg-card/60 opacity-80" : "bg-card font-medium border-l-4 border-l-amber-600"
                }`}
              >
